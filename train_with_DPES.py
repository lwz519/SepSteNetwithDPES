...
from model import SepSteNet_with_DPES
...
def train(x_train_data, re_x_train_data, y_train_data, x_val_data, re_x_val_data, y_val_data, weight_path, model):
    ...
    hist = model.fit([x_train_data, re_x_train_data], y_train_data, batch_size=args.batch_size, epochs=args.epoch,
                     validation_data=([x_val_data, re_x_val_data], y_val_data), callbacks=callbacks_list, verbose=2, shuffle=True)


def test(x_test_data, re_x_test_data, y_test_data, weight_path):
    in_shape = x_test_data.shape[1:]
    net = SepSteNet_with_DPES(in_shape)
    net.load_weights(weight_path)
    y_predict = net.predict([x_test_data, re_x_test_data])
...
def main(arg):
    ...
    x_train, re_x_train, y_train = np.asarray([item[0] for item in tmp]), np.asarray([item[1] for item in tmp]), np.asarray([item[2] for item in tmp])
    x_train = x_train.astype(np.float)
    re_x_train = re_x_train.astype(np.float)

    tmp = np.load('./dataset/data_{}_{}s_{}_val.npy'.format(method, arg.length, arg.em_rate), allow_pickle=True)
    x_val, re_x_val, y_val = np.asarray([item[0] for item in tmp]), np.asarray([item[1] for item in tmp]), np.asarray([item[2] for item in tmp])
    x_val = x_val.astype(np.float)
    re_x_val = re_x_val.astype(np.float)

    tmp = np.load('./dataset/data_{}_{}s_{}_test.npy'.format(method, arg.length, arg.em_rate), allow_pickle=True)
    x_test, re_x_test, y_test = np.asarray([item[0] for item in tmp]), np.asarray([item[1] for item in tmp]), np.asarray([item[2] for item in tmp])
    x_test = x_test.astype(np.float)
    re_x_test = re_x_test.astype(np.float)
    ...
    model = SepSteNet_with_DPES(in_shape)
    model.summary()

    if arg.train:
        print('[INFO] Training started')
        train(x_train, re_x_train, y_train, x_val, re_x_val, y_val, arg.model_path, model)
        print('[INFO] Finished')
    if arg.test:
        print('[INFO] Testing the detection performance')
        test(x_test, re_x_test, y_test, arg.model_path)
        print('[INFO] Finished')


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='SepSteNet with DPES')
    ...
    if args.method[:4] == 'Miao':
        ...
        method = args.method
    args.model_path = './model_weights/SepSteNet_with_DPES/SepSteNet_with_DPES_{}_{}s_{}.h5'.format(method, args.length, args.em_rate)
